...
input:
    ...
    * tau: the softness of interest selection
    * linear_size: size of linear layers in BaM_l
returns:
...
class MIND(BasicModel):
    ...
    # initialization of model
    def __init__(self, item_num, hidden_size, batch_size, interest_num=4, seq_len=50, routing_times=3, relu_layer=True, selection='hard', T=1, linear_size = 16):
        super(MIND, self).__init__(item_num, interest_num, hidden_size, batch_size, seq_len, selection, T, linear_size)
        self.routing_times = routing_times    
    ...
    def forward(self, item_list, pos_list, mask, train=True):
        ...
        return interest_eb, selection, None